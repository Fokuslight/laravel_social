            ...
            <div class="flex">
                ...
                <div class="ml-2 relative">
                    ...
                    <input type="text" name="search" placeholder="search"
                           class="text-sm pl-8 focus:outline-none w-56 rounded-full bg-gray-200 h-8">
                </div>
            </div>
        </div>
        <div class="w-1/3 flex justify-center items-center h-full">
            <router-link to="/" class=" h-full px-6 flex items-center border-b-2 border-blue-500">
            ...
            <router-link :to="'/users/' + authUser.data.user_id" class=" h-full px-6 flex items-center border-b-2 border-white h-full">
                <img class="w-8 h-8 object-cover rounded-full"
                     :src="authUser.data.attributes.profile_image.data.attributes.path"
                     alt="avatar">
            </router-link>
            <router-link to="" class=" h-full px-6 flex items-center border-b-2 border-white h-full">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="fill-current w-5 h-5">
                    <path
